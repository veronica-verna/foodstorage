...
myPlot <- function(prepare,
  ...
  if (ncol(data) == 3) {
    if (smoother[1] == "loess") {
      lo <- loess(data[,3] ~ Tag_Nr, span = span[1])
      preds <- predict(lo)
    }
    
    # Was liegt vor?
    if (colnames(data)[3] == "Verzehr") {
      Verzehr <- data[,3]
      # bauen eines leeren 'Bilderrahmens'
      if (par == TRUE) par(mfrow = mfrow, mar = mar)
      if (length(prepare) == 2) {
        plot(x = Datum, 
             y = Verzehr, 
             xaxt = "n", 
             ...
             pch = pch[1], 
             main = paste("Verzehr von", name.of.product, sep=" "), 
             col = col_points[1])
      }
      ...
      if (length(prepare) == 3) {
        plot(x = Datum, 
             ...
             ylim = c(range(preds)[1] - 0.2, range(preds)[2] + 0.2), 
             pch = pch[1], 
             main = paste("Verzehr von", name.of.group, sep=" "), 
             col = col_points[1])
      }
      
      
      # jetzt erst wird gezeichnet...
      lines(x = Datum, y = preds, col = col_line[1], lwd = lwd[1], lty = lty[1])
      
      # beschriften der y-Achse
      axis(side = axis_side[1], las = las[1])
      mtext(text = "Verzehr in Kilo", side = axis_side[1], line = line[1])
    } 
    
    if (colnames(data)[3] == "Warenbestand") {
      Warenbestand <- data[,3]
      # bauen eines leeren 'Bilderrahmens'
      if (par == TRUE) par(mfrow= mfrow, mar = mar)
      if (length(prepare) == 2) {
        plot(x = Datum, 
             ...
             ylab = "", 
             xlab = "", 
             ylim = c(range(preds)[1] - 0.5, range(preds)[2] + 0.5), 
             pch = pch[1], 
             main = paste("Warenbestand von", name.of.product, sep=" "), 
             col = col_points[1])
      }
      ...
      if (length(prepare) == 3) {
        plot(x = Datum, 
             ...
             ylim = c(range(preds)[1] - 0.5, range(preds)[2] + 0.5), 
             pch = pch[1], 
             main = paste("Warenbestand von", name.of.group, sep=" "), 
             col = col_points[1])
      }
  ...
  if (ncol(data) == 4) {
    ...
    if (smoother[1] == "loess") {
      lo_waren <- loess(data[,3] ~ Tag_Nr, span = span[1])
      preds_waren <- predict(lo_waren)
    }
  ...
  # Ausführung der Beschriftung
  axis(1, at=date1st, labels=month.abb[month(as.POSIXlt(date1st, format="%Y-%m-%d"))]) # as.posix gibt den Monat in Ziffern aus, month.abb wandelt Ziffern in erste drei Buchstaben um; month würde ausgeschriebene Monate ausgeben    
